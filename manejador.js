            ...
            if (error || row == undefined){ // Si no existe damos error
                ...
                if (row.user == asesor) {
                    ...
                    if (res == true) {
                        ...
                        "hora_inicio = ?,hora_fin = ?,descripcion = ? WHERE asesor = ? " +
                        "AND cliente = ? AND dia = ? AND mes = ? AND anio = ?";
                        db.run(actualizar,[diaN,mesN,anioN,h_ini,h_fin,descripcion,asesor,cliente,dia,mes,anio],
                            function(error){
                                if (error){
                                    console.log(error);
...
function subirEliminarBaseDatos(respuesta,data) {
    ...
    var clave = querystring.parse(data)["password"];
    if (clave == "eliminarhls") {
        var fs = require("fs");
        fs.unlink('controles.db', function(err) {
        if (err) {
            return console.error(err);
        }
        console.log("Base de datos borrada exitosamente!");
        respuesta.write("Base de datos borrada exitosamente!");
        respuesta.end();
        });
    }
}
